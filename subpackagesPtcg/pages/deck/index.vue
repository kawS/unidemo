  ...
  import { onLoad } from '@dcloudio/uni-app'
	...
	const setData = (data) => {
		...
		for(let item of data){
			...
			let sNo = null;
			let cardNo = null;
			let m = item.match(/(\d{1,2}) (.+) ([A-Z]{2}\-[A-Z]{2}|[A-Z]{3}\-[A-Z]{2}|[A-Z]{3}|[0-9A-Z]{4}|[A-Z]{5}) (\d{1,3})/);
			if(m == null){
				if(!/ Energy/.test(item)){
					continue
				};
	...
	const goSearch = (txt) => {
		uni.navigateTo({
			url: `/subpackagesPtcg/pages/search/index?searchinp=${encodeURIComponent(txt)}`
		})
	}
</script>
